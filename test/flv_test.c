...
static void adtsHeader(char *adts_header_buffer, int data_len, int profile, int sample_rate_index, int channels){
    ...
    adts_header_buffer[1] = 0xf0;
    adts_header_buffer[1] |= (0 << 3);
    adts_header_buffer[1] |= (0 << 1);
    adts_header_buffer[1] |= 1;

    adts_header_buffer[2] = (profile)<<6;
    adts_header_buffer[2] |= (sample_rate_index & 0x0f)<<2;
    adts_header_buffer[2] |= (0 << 1);
    adts_header_buffer[2] |= (channels & 0x04)>>2;

    adts_header_buffer[3] = (channels & 0x03)<<6;
    adts_header_buffer[3] |= (0 << 5);
    adts_header_buffer[3] |= (0 << 4);
    adts_header_buffer[3] |= (0 << 3);
    adts_header_buffer[3] |= (0 << 2);
    adts_header_buffer[3] |= ((adts_len & 0x1800) >> 11);

    adts_header_buffer[4] = (uint8_t)((adts_len & 0x7f8) >> 3);
    adts_header_buffer[5] = (uint8_t)((adts_len & 0x7) << 5);
    adts_header_buffer[5] |= 0x1f;
...
void videoCallBack(enum FLVVideoType type, int64_t timestamp, uint8_t* data, uint32_t data_len, void* arg){
    ...
    if(!video_ready){
        if(type == FLV_VIDEO_H264){
            ...
            if(nalu_type == 7){ // sps
                ret = setVideoParameters(context_muxer, NULL, 0, data, data_len, NULL, 0);
                if(ret < 0){
                    printf("setVideoParameters error\n");
                }
            }
            if(nalu_type == 8){ // pps
                ret = setVideoParameters(context_muxer, NULL, 0, NULL, 0, data, data_len);
                if(ret < 0){
                    printf("setVideoParameters error\n");
                }
                video_ready = 1;
            ...
            if(nalu_type == 32){ // vps
                ret = setVideoParameters(context_muxer, data, data_len, NULL, 0, NULL, 0);
                if(ret < 0){
                    printf("setVideoParameters error\n");
                }
            }
...
int main(int argc, char **argv){
    ...
    ret = writeFLVGlobalHeader(context_muxer, 1, 1);
    if(ret < 0){
        printf("writeFLVGlobalHeader error\n");
    }
    AMFDict dict;
    ...
    printAMFDict(dict);
    ret = writeScriptData(context_muxer, 0, dict);
    if(ret < 0){
        printf("writeScriptData error\n");
    }
#endif
    ret = demuxerFLVFile(context_demuxer, input);
    if(ret < 0){
        printf("demuxerFLVFile error\n");
    }
